...
import { createIdempotencyKey, type PipelinePostOptions } from "../../../shared/api/apiClient.js";
...
function shouldRotateKeyOnError(error: unknown): boolean {
...
export function useIdempotentReleaseAction() {
  ...
  const run = useCallback(
    async <T>(intent: string, action: (options: PipelinePostOptions) => Promise<T>): Promise<T> => {
  ...
  const abandon = useCallback((intent: string) => {
    keysRef.current.delete(intent);
  }, []);
