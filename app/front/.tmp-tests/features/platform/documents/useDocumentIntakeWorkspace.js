...
import { listAllDocuments, listVariants, normalizeDocuments, uploadDocument, } from "../platformApi.js";
...
function isBulkSelectableRevision(revision) {
...
export function useDocumentIntakeWorkspace() {
    ...
    const [documents, setDocuments] = useState(projectId ? { status: "loading" } : { status: "no-project" });
    const [variants, setVariants] = useState(projectId ? { status: "loading" } : { status: "idle" });
    ...
    const [lastUploadedRevisionId, setLastUploadedRevisionId] = useState(null);
    ...
    const load = useCallback(async (pid, signal) => {
        ...
        try {
            ...
            setDocuments(revisions.length === 0 ? { status: "empty" } : { status: "ready", revisions });
    ...
    const toggleRevision = useCallback((revisionId) => {
        setNotice(null);
    ...
    const selectAllRevisions = useCallback(() => {
        setNotice(null);
        setSelectedRevisionIds(documents.status === "ready"
            ? new Set(documents.revisions
    ...
    const upload = useCallback(async (file, sourceRelpath) => {
        if (!projectId) {
            return false;
        }
        setUploading(true);
    ...
    const canNormalize = selectedVariantId !== null && selectedRevisionIds.size > 0 && !normalizing;
    const bulkSelectableRevisionIds = documents.status === "ready"
        ? documents.revisions
            .filter(isBulkSelectableRevision)
            .map((revision) => revision.source_document_revision_id)
        : [];
    const bulkSelectableRevisionCount = bulkSelectableRevisionIds.length;
    const allBulkSelectableSelected = bulkSelectableRevisionCount > 0 &&
