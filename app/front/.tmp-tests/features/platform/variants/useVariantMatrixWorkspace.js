...
import { createVariant as apiCreateVariant, getVariantMatrix, listVariants, } from "../platformApi.js";
...
export function useVariantMatrixWorkspace() {
    ...
    const [matrix, setMatrix] = useState(projectId ? { status: "loading" } : { status: "no-project" });
    ...
    const selectedCell = useMemo(() => cells.find((cell) => cell.cell_id === selectedCellId && cell.buildable) ?? null, [cells, selectedCellId]);
    ...
    const submit = useCallback(async () => {
        ...
        try {
            ...
            if (mapped.status === 409 && mapped.code === "STALE_VARIANT_MATRIX_CELL") {
                ...
                setNotice({
                    tone: "warning",
                    message: "La configuración cambió y la celda seleccionada quedó obsoleta. Revisa la matriz actualizada y vuelve a confirmar una celda.",
                });
                runLoad(projectId);
            }
            else {
