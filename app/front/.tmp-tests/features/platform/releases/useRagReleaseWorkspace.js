...
import { buildRelease, createReleaseDraft, getConfiguration, getRelease, listCorpusSnapshots, listReleases, listVariants, publishRelease, retireRelease, validateRelease, } from "../platformApi.js";
...
export function useRagReleaseWorkspace() {
    ...
    const [load, setLoad] = useState(projectId ? { status: "loading" } : { status: "no-project" });
    const [buildReport, setBuildReport] = useState({ status: "idle" });
    ...
    const [draftSnapshotId, setDraftSnapshotId] = useState(null);
    const [draftBindingKey, setDraftBindingKey] = useState(null);
    ...
    const fetchAll = useCallback(async (pid, signal) => {
    ...
    const applyRelease = useCallback((updated) => {
        setLoad((current) => {
            if (current.status !== "ready") {
                return current;
            }
            const releases = current.data.releases.some((r) => r.rag_release_id === updated.rag_release_id)
                ? current.data.releases.map((r) => r.rag_release_id === updated.rag_release_id ? updated : r)
    ...
    const handleMutationError = useCallback(async (error, releaseId) => {
        const mapped = mapPipelineError(error);
    ...
    const build = useCallback(async () => {
        ...
        setNotice(null);
        try {
            const report = await idempotent.run(`build:${releaseId}`, (options) => buildRelease(releaseId, options));
    ...
    const validate = useCallback(async () => {
        ...
        setNotice(null);
        try {
            const updated = await idempotent.run(`validate:${releaseId}`, (options) => validateRelease(releaseId, options));
    ...
    const publish = useCallback(async () => {
        ...
        setNotice(null);
        try {
            const updated = await idempotent.run(`publish:${releaseId}`, (options) => publishRelease(releaseId, options));
            applyRelease(updated);
    ...
    const retire = useCallback(async (reason) => {
        if (!selectedRelease || busyAction) {
            return false;
        }
        ...
        setNotice(null);
        try {
            const updated = await idempotent.run(`retire:${releaseId}`, (options) => retireRelease(releaseId, { reason: trimmed }, options));
            applyRelease(updated);
    ...
    const selectRelease = useCallback((releaseId) => {
    ...
    const canCreateDraft = !creating && draftVariantId !== null && draftSnapshotId !== null && draftBindingKey !== null;
    return {
        projectId,
        load,
        selectedReleaseId,
