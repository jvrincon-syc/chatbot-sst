...
def main() -> int:
    ...
    if args.json:
        ...
        print(f"OCRmyPDF: {'enabled' if report.ocrmypdf_enabled else 'optional'} ({report.ocrmypdf_version or 'unavailable'})")
        ...
        print(f"Ghostscript: {'required' if report.ocrmypdf_enabled else 'optional'} ({report.ghostscript_version or 'unavailable'})")
        if report.issues:
