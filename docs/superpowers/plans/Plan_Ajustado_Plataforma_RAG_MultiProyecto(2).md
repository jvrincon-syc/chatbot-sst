...
- [x] Inventariar, en PostgreSQL real, conteos y hashes de `indexing_normalized_documents`, `chunk_bundles`, `embedding_bundles`, `embedding_runs`, `indexing_runs`, `indexing_nodes`, `idx_vec_*` y `retrieval_profiles` antes de cualquier migración. — **Evidencia:** `scripts/rag_platform/inventory_baseline.py` (solo lectura, reusa `build_dsn_from_env`) ejecutado contra `chatbot_sst`; salida verbatim archivada en `docs/rag-platform/migration-baseline.md` §"Inventario PostgreSQL real" (39 normalized, 40 chunk_bundles, 15 embedding_bundles, 18 filas en `idx_vec_local_bge_m3_v1`, digests de contenido incluidos).
- [x] Verificar los nombres reales de constraints, PKs e índices en la base que se migrará; no asumir que todos los entornos coinciden solo por los archivos SQL. — **Evidencia:** mismo script emite constraints/PK/índices reales por tabla desde `pg_constraint`/`pg_indexes`; archivados en el mismo doc. Coinciden con los `.sql` del baseline (unicidades legacy confirmadas), verificación registrada en la nota final de esa sección.
...
- [x] Extender el contrato Schema 2.0 para datos nuevos con `project_id`, `source_document_revision_id`, `normalized_document_id`, `processing_profile_id` y fingerprints; conservar un adaptador explícito Schema 2.0 legacy para SST. — **Evidencia:** `ingestion/schemas/artifacts.py` añade `PlatformDocumentIdentity` (IDs validados por prefijo tipado, sin importar `rag_platform` para no acoplar legacy) y el campo **opcional** `MetadataArtifact.platform_identity` (default `None` = adaptador legacy: los artefactos SST validan sin cambios). Pruebas en `tests/ingestion/test_identity.py::test_metadata_platform_identity_is_optional_for_legacy` y `::test_platform_document_identity_*`.
...
- [x] Resolver `DocumentType` contra el catálogo y la política versionados del proyecto. El Literal actual solo permanece en el adaptador SST/legacy hasta que la validación de política esté cubierta. — **Evidencia:** `rag_platform/domain/classification.py::resolve_document_type` valida el `code` contra `ProjectConfiguration.document_types` fail-closed (lanza `DocumentTypeNotPermitted`, error de dominio nuevo en `errors.py`). El `Literal` legacy queda confinado al adaptador SST. **Desajuste resuelto:** la plantilla SST del catálogo (`project_service.py::_SST_DOCUMENT_TYPES`) se alineó con las 15 etiquetas que emite el clasificador legacy (antes 5, con `formato` en vez de `formulario`), para que una clasificación SST real no se rechace. Pruebas: `tests/rag_platform/test_document_type_policy.py::test_resuelve_*`, `::test_rechaza_*`.
- [x] Extraer reglas SST de `ingestion/classification/rules.py` hacia una política cargada desde el snapshot de configuración; conservar el adaptador que produce exactamente las decisiones SST actuales. — **Evidencia (enfoque aditivo mínimo, decisión del usuario):** puerto `ClassificationPolicy` + caso de uso `ClassifyProjectDocumentUseCase` en `application/classification_service.py`; adaptador `infrastructure/classification/sst_policy.py::SstClassificationPolicy` **envuelve** `classify_document` sin reimplementar reglas (`rules.py` no se reescribe). **La política se carga desde el snapshot de configuración** con `infrastructure/classification/policy_factory.py::build_classification_policy(configuration)`, que deriva el adaptador SST del catálogo versionado del proyecto y **falla cerrado** (`NoClassificationPolicyConfigured`) si la config no resuelve política, sin degradar a un default. Pruebas: `test_document_type_policy.py::test_cargador_deriva_politica_sst_del_snapshot`, `::test_cargador_falla_cerrado_sin_taxonomia_con_motor`, paridad exacta `::test_adaptador_sst_reproduce_la_decision_legacy`, composición `::test_caso_de_uso_*`.
...
- [ ] Crear `rag_build_runs` y `rag_build_steps` como ledger durable para todas las etapas y reusos. El run sí apunta a release; el bundle físico no.
- [ ] Cambiar el repositorio filesystem para almacenar bundles sellados bajo `data/projects/{project_id}/chunks/{chunk_bundle_id}/`, con manifest, checksums, parents y children. `latest` puede existir como vista de UI por proyecto, pero ninguna release puede depender de él.
- [ ] Mantener `replace()` sin cambios para el flujo legacy y añadir `stage_and_seal()` para la plataforma; el nuevo adaptador no puede llamar a `replace()` sobre una ruta compartida.
- [ ] Añadir `project_id`, `normalized_document_id`, profile fingerprint y estado de sellado a `chunk_bundles`. Sustituir la unicidad global de `bundle_fingerprint` por la identidad física definida en la sección 4.4.
- [ ] Mantener `corpus_version` como columna legacy; no incluirlo en la identidad de bundles nuevos.
- [ ] Hacer que la entrada de build de plataforma reciba solo `rag_release_id`; el backend resuelve snapshot y perfil de chunking. `ChunkingRunRequest` y la API legacy conservan su payload actual.
- [ ] Registrar por cada reuso `exact_identity`, `revalidated_compatibility` o `operator_approved`, junto con el artefacto origen. `operator_approved` no puede salvar incompatibilidad de dimensión, métrica o proyecto.
